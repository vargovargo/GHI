...
setwd("~/GHI/methodsPaper")
NHTS <- readRDS(file = "../R/data/NHTS.rds")
head(NHTS)
@ 

As with the original ITHIM implementation we employ a comparative risk
analysis (CRA) across scenarios defined by mean active transportation
time, i.e., walking and cycling.  To do so we estimate the
proportional change in disease burden between a baseline model and an
alternative scenario, namely the population attributable fraction,
$\af$.  The population attributable fraction is defined below and
computed for each disease included in the model.
