...
getTractAgeSex <- function(state, county) {
  ...
  ACSpop <- ACSpop %>% select(state,county,tract,sex,age,value) %>% group_by(state,county,sex,age) %>% summarise(n = sum(as.integer(value))) %>% ungroup()
...
getTractAgeSex(state = 55,county = 025)
