...
stateVec <- c("CA","TX","NY","FL","WI","MI") #c("CA","TX","NY","FL","VA","NC","AZ")
stateVecOrdered <- c("TX","FL","NY","CA","WI","MI") #c("NC","TX","VA","FL","AZ","NY","CA")
...
ITHIM.default <- updateITHIM(ITHIM.default, "muNonTravelMatrix", METmatrix)
...
MItoCA <- data.frame(lapply(compareModels(ITHIM.MI,ITHIM.CA)$daly.delta, function(x) sum(x$M + x$F)), baseline = "MI", scenario = "CA")
WItoCA <- data.frame(lapply(compareModels(ITHIM.WI,ITHIM.CA)$daly.delta, function(x) sum(x$M + x$F)), baseline = "WI", scenario = "CA")
MItoNY <- data.frame(lapply(compareModels(ITHIM.MI,ITHIM.NY)$daly.delta, function(x) sum(x$M + x$F)), baseline = "MI", scenario = "NY")
WItoNY <- data.frame(lapply(compareModels(ITHIM.WI,ITHIM.NY)$daly.delta, function(x) sum(x$M + x$F)), baseline = "WI", scenario = "NY")

D <- melt(rbind(MItoCA,WItoCA,MItoNY,WItoNY))
...
p + theme_bw() + scale_y_continuous(name="DALYs Averted (thousands)") + facet_grid( . ~ baseline) + theme(axis.text.x  = element_text(angle = 90, vjust = 0.5, size = 8))

#+ scale_x_discrete(name="", labels = c("0-4","5-14","15-29","30-44","45-59","60-69","70-79","80+")) + theme(axis.text.x  = element_text(angle = 270, vjust = 0.5, size = 8)) + facet_grid( . ~ disease, scales = "free_y")
```
