...
atVec <- seq(35, 120, length.out = 10)
muNonTravelVec <- c(1, 5, 10)
DTotal <- data.frame()
...
DTotal <- within(DTotal, muNonTravel <- factor(muNonTravel, levels = c(1,5,10), labels = c("low","medium","high")))
DTotal <- subset(DTotal, quants == "percentiles")


p <- ggplot(DTotal, aes(x = at, y = -DALYs/1e3, colour = muNonTravel), colour = muNonTravel)
p + geom_line() + geom_point() + theme_bw() + scale_y_continuous(name="DALYs Averted Nationwide (Thousands)") + scale_x_continuous(name="Active Transport (minutes/week) (Referent Group)") + theme(axis.text.x  = element_text(angle = 90, vjust = 0.5, size = 8)) #+ facet_grid( . ~ muNonTravel)


#p + stat_smooth( method = loess) + geom_point() + theme_bw() + scale_y_continuous(name="DALYs Averted Nationwide (Thousands)") + scale_x_continuous(name="Active Transport (minutes/week) (Referent Group)") + theme(axis.text.x  = element_text(angle = 90, vjust = 0.5, size = 8)) #+ facet_grid( . ~ muNonTravel)
...
p + theme_bw() + scale_x_discrete(name="", labels = c("0-4","5-14","15-29","30-44","45-59","60-69","70-79","80+")) + scale_y_continuous(name="Nationwide Burden (thousands)") + theme(axis.text.x  = element_text(angle = 90, vjust = 0.5, size = 8)) + facet_grid( . ~ disease, scales = "free_y")
```
