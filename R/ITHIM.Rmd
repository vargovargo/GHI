...
for (HHSTATE in stateVec){
    ...
    ITHIMList[[HHSTATE]] <- updateITHIM(ITHIMList[[HHSTATE]], "GBD", gbdList[[HHSTATE]])
  }
...
ITHIMListScenario <- lapply(ITHIMList, function(ithim) updateITHIM(ithim, parName = "muwt", parValue = ithim$parameters$muwt + 10*7)) 
...
foo <- subset(foo, state %in% c("WI", "MI","OH"))
...
p + geom_bar(stat= "identity") + scale_x_discrete(name="") + scale_y_continuous(name="Thousands of DALYs") + guides(fill=FALSE) + scale_fill_manual(values=cbPalette) + coord_cartesian(ylim = c(175, 600)) + theme(axis.text.y  = element_text(size = 16)) + theme(axis.text.x  = element_text(size = 16), axis.title.x  = element_text(size = 16)) + coord_flip() 
