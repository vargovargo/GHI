...
```{r AF, echo = FALSE}
lapply(AF[-(which(names(AF)%in%c("CVD","Diabetes")))], function(x) 100*round(x[-(1:2),],4))
```
### Relative Risk Model
```{r explore}
...
phi <- function(x,k=0.5,phi0=RR$BreastCancer$F[1,1]) phi0^{x^k}
...
```{r plot2, echo = FALSE}
D <- melt(ITHIM.baseline$parameters$Rwt, c("age","sex"), value.name = "rhowt")
