...
cps %>% merge(METs, by.y = "row.names", by.x = "TUCASEID") %>% 
...
parameters <- cps %>% merge(METs, by.y = "row.names", by.x = "TUCASEID") %>% group_by(SEX, AGECAT) %>% 
summarise(mean = mean(METs, na.rm=T), sd = sd(METs, na.rm=T))
