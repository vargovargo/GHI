...
PERV2PUB <- read.csv(file = "~/GHI/data/NHTS/Ascii/PERV2PUB.CSV", na.strings = c("-7","-8","-9"), stringsAsFactors = FALSE)
...
person.df <- with(NHTS.person,{
    data.frame(houseID=HOUSEID, subjectID=PERSONID, sex = SEX, age = AGE, location = MSA_ID, income = HHFAMINC, nwalktrp = NWALKTRP, nbiketrp = NBIKETRP)
    })
...
```{r TravelSurvey2, eval = TRUE, fig.width = fig.width, echo = FALSE}
NHTS.df <- dplyr::left_join(person.df, trip.df, by = c("houseID","subjectID"))
