...
```{r TravelSurvey2, eval = compute, fig.width = 10, echo = FALSE}
NHTS.TravelSurvey <- readRDS(file = "~/GHI/R/data/NHTS.TravelSurvey.rds")

NHTS.df <- dplyr::left_join(NHTS.TravelSurvey@person, NHTS.TravelSurvey@trip, by = c("houseID","subjectID"))

NHTS.df <- within(NHTS.df, duration <- ifelse(is.na(duration), 0, duration))
