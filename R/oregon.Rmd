...
```{r misc320, message = FALSE, eval = TRUE, warning = FALSE, error = TRUE, echo = TRUE}
ITHIM.baseline <- createITHIM(activeTransportFile = "~/Downloads/activeTransportTime.csv",
                              GBDFile = "~/Downloads/gbd.csv")
```

The contents of the _ITHIM_ object are displayed in summary when the
object is printed.  See below.

```{r show, message = FALSE, eval = TRUE, warning = FALSE, error = TRUE, echo = TRUE}
ITHIM.baseline
```

Currently the list of internal model parameters resides in the help page for the
ITHIM class (`help("ITHIM")`).  Alternatively one can use the method
`getParameterNames` to display the names.
...
If we wanted to examine the overal means in the model we could use the
method `getMeans`, which returns a vector of means.  To find a list of
available methods use `getMethodsITHIM` with no arguments.
...
## Creating a Scenario ITHIM object

Suppose we wanted to create a scenario in which the mean cycling time
and mean walking time are doubled.  We simply need to replace the
active transport file with one that has the desired values.

Sample Files
* [activeTransportTime_scenario.csv](https://ithim.wiscweb.wisc.edu/wp-content/uploads/sites/110/2017/06/activeTransportTime_scenario.csv)


Please update the path and names in the code below to match the csv
files you wish to use.

```{r misc20, message = FALSE, eval = TRUE, warning = FALSE, error = TRUE, echo = TRUE}
ITHIM.scenario <- createITHIM(activeTransportFile = "~/Downloads/activeTransportTime_scenario.csv",
                              GBDFile = "~/Downloads/gbd.csv")
```

```{r misc7620, message = FALSE, eval = TRUE, warning = FALSE, error = TRUE, echo = FALSE}
modelMeans <- rbind(cbind(rbind(getWalkTime(ITHIM.baseline), getCycleTime(ITHIM.baseline), getNonTravelMETs(ITHIM.baseline)),vision = "baseline"), cbind(rbind(getWalkTime(ITHIM.scenario), getCycleTime(ITHIM.scenario), getNonTravelMETs(ITHIM.scenario)),vision = "scenario"))

p <- ggplot(modelMeans, aes(x = ageClass, y = mean, fill = vision))
p + geom_bar(stat = "identity", position = "dodge") + facet_grid(type ~ sex, scales = "free_y")  + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
In the above example we needed only to change two means in the model
and nothing else.  To do so we could avoid creating input files by
simply updating the baseline object with a few new parameters.  (note:
this is the overall mean for the whole population. From this, _ITHIM_
will create means for each age & gender strata)


We can use the mode-specific means from the _meanVec_ vector and
...
scenario. Recall, we're interested in a scenario that has double the
mean walking time and double the mean cycling time of the baseline
scenario.
...
## Creating an ITHIM object for Portland (MSA 6442)
...
```{r gbd, eval = TRUE, echo = TRUE, fig.width = fig.width, fig.asp = 1/phi}
```
