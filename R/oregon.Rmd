...
parameters.
(note: this is the overall mean for the whole population. From this, _ITHIM_ will create means for each age & gender strata)
...
Next we'll create a list of the parameters we wish to change in our scenario. Recall, we're interested in a scenario that has double the mean walking time and double the mean cycling time of the baseline scenario.
...
compute the change in burden due to physical activity as follows. We provide a few examples to give you an idea of options for diseases and burden. 
...
We use the National Household Transportation Survey (NHTS) data stored as a _TravelSurvey_ R object to estimate mean walk and cycle times.
...
```{r sessionInfo, eval = FALSE, echo = TRUE, results = "show", warning = FALSE, error = FALSE, message = FALSE}
library("testthat")
test_package("ITHIM")
sessionInfo()
```
